...
class PatientController extends Controller
{
    public function search(Request $request): JsonResponse
    {
        $patients = Patient::select(DB::raw('id, concat(name, " ",last_name, " ", mother_last_name) full_name,
            if(has_medic_history is not null, concat("'.config('app.url').'patients/generate-medical-history-pdf/", patients.id), null) as uri'))
    ...
    public function generateMedicalHistoryPdf($patientId)
    {
        ...
        return $pdf->stream('historial_medico-'.now().'.pdf');
    }
