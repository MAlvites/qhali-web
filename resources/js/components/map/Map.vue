<template>
    <div>
        <div class="class-position">Mapas</div>
        <v-card
                max-width="1000"
            ...
            <v-card-title>
                ...
                ></v-text-field>
                <v-spacer></v-spacer>
                <v-btn @click="openModal" class="primary">Agregar Mapa</v-btn>
            </v-card-title>
            <v-data-table
            ...
            >
                <template v-slot:[`item.url`]="{ item }">
                    <a :href="item.url" :alt="item.name" target="_blank">Enlace</a>
                </template>
...
<script>
import addEditRobot from './addEditMap';
...
export default {
    ...
    methods: {
        ...
        editUser() {
            axios.post('/maps/update', this.formRobot)
            .then(response => {
                this.allData()
        ...
        saveUser() {
            axios.post('/maps/create', this.formRobot)
            .then(response => {
                this.allData()
                alerts.alertSwal(response.data.message, 'success')
                this.showModal = false
        ...
        allData() {
            ...
            axios.post('/maps/search?page='+this.page, json)
            .then(response => {
